---
type: video-note
media-link: https://www.youtube.com/watch?v=_S7123H8o3E
autor: Prof. Palazzo
publicado-em: 2020-09-07
visto-em: 2026-08-12T13:37:00
area: Tecnologia da Informação
tags:
  - engenharia_de_software
  - sistemas_de_informacao
  - modelage_de_sistemas
  - complexidade
  - arquitetura_de_sistemas
---


![Complexidade de Sistemas](https://www.youtube.com/watch?v=_S7123H8o3E)

> [!INFO] Resumo A tese central do vídeo é que a complexidade em sistemas de informação não reside apenas na complexidade algorítmica, mas na dificuldade intrínseca de compreender, modelar e representar a realidade complexa e dinâmica do mundo real em sistemas computacionais.

> [!NOTE] Definição Central
> 
> > **Complexidade de Sistemas de Informação:** A dificuldade que um problema apresenta para ser compreendido e modelado de forma satisfatória, envolvendo a multiplicidade de dados e relações intrincadas que precisam ser representadas.

> [!QUOTE] Citação Importante
> 
> > "O projeto de sistemas de informação é uma atividade complexa. Esta complexidade é consequência da dificuldade de compreensão da realidade." — Prof. Palazzo

## 1. Definição e Natureza da Complexidade

_A complexidade aumenta à medida que os sistemas evoluem e envolvem mais atores e interdependências. Não se trata apenas de código, mas de entendimento._

- [00:00](https://www.youtube.com/watch?v=_S7123H8o3E#t=0) **Diferenciação:** A complexidade em sistemas de informação difere da complexidade de algoritmos; é um desafio de representação do real.
- [02:29](https://www.youtube.com/watch?v=_S7123H8o3E#t=149) **Trabalho Multidisciplinar:** A necessidade de grupos multidisciplinares e cooperação para gerir a complexidade crescente.

## 2. A Relação entre Realidade e Sistemas

_Como o software interage (ou não) com o mundo real e a importância do modelo interno._


- [03:07](https://www.youtube.com/watch?v=_S7123H8o3E&t=187#t=03:07.14) **Controle de complexidade:** Estruturação,Abstração,Formalização

![[controle_complexidade.jpg|Complexidade de Sistemas de Informação - 03:07]]

- [04:07](https://www.youtube.com/watch?v=_S7123H8o3E#t=247) **[[Sistemas de Informação]]:** O sistema mantém um modelo interno para processar dados que refletem o mundo real (independente e não reativo).

![[modelo_sistema_info.jpg|Complexidade de Sistemas de Informação - 04:08]]

![[sistema_info.jpg]]

- [05:53](https://www.youtube.com/watch?v=_S7123H8o3E#t=353) **[[Sistemas de Controle]]:** O sistema atua sobre o mundo real, que é reativo aos estímulos do controlador.

![[modelo_sistema_control.jpg|Complexidade de Sistemas de Informação - 05:51]]

(A diferença ao anterior é a flecha vermelha-Reativo)

![[sistema_control.jpg|Complexidade de Sistemas de Informação - 06:08]]

## 3. Engenharia de Sistemas e Processo de Desenvolvimento

_A metodologia necessária para converter a realidade em sistemas funcionais._

- [06:46](https://www.youtube.com/watch?v=_S7123H8o3E#t=406) **Decomposição:** Passo a passo para assimilar a experiência e transformar dados brutos em dados fundamentais.
- [08:04](https://www.youtube.com/watch?v=_S7123H8o3E&t=485#t=08:04.95) **[[Software]]**: Codificar conhecimento humano em uma linguagem computacional
- [10:36](https://www.youtube.com/watch?v=_S7123H8o3E#t=636) **Engenharia:** O fluxo desde a análise da realidade, criação de modelo, validação com usuários, até a aceitação do sistema.

---

## 🎯 Principais Pontos (Takeaways)

_Quais foram os aprendizados mais importantes? O que pode ser aplicado na prática?_

- A complexidade é um problema de compreensão da realidade, não apenas de lógica de programação.
- A modelagem correta é essencial, mas nenhuma metodologia garante um "bom software" automaticamente; o sucesso depende da precisão da especificação humana.
- O desenvolvimento de sistemas é uma tarefa multidisciplinar que exige colaboração técnica e domínio do especialista da área de negócio.

---

## 🔗 Conexões e Conceitos Relacionados

_Links para outras notas no Obsidian._

- [[Modelagem de Dados]]
- [[Engenharia de Software]]
- [[Sistemas de Controle e Feedback]]

---

## 📚 Referências Citadas no Vídeo

_Livros, artigos, ferramentas, ou autores mencionados._

- Estudo próprio de 2011 sobre programas de pós-graduação e cooperação interna.
- Conceitos gerais de Engenharia de Sistemas de Informação.